...
function links(){
	...
	if(empty($r->github)){
		$github = "<a href='#' data-target='#github' data-toggle='modal'>Add GitHub <i class='fa fa-plus-circle'></i></a>";
	} else {
		$github = "<a href='#' data-target='#github' data-toggle='modal'>Update GitHub <i class='fa fa-pencil'></i></a>";
	}
	if(empty($r->linkedin)){
...
function user_info(){
	...
	if(empty($r->github)){
		...
		$github = "<a href='$r->github' target='_blank'><i class='fa fa-github'></i> Connected</a>";
	} 
	if(empty($r->linkedin)) {
		$linkedin = "<a href='#' data-target='#linkedin' data-toggle='modal'>Add Linkedin <i class='fa fa-plus-circle'></i></a>";
	} else {
...
function percentage() {
	...
	if(!empty($name) && !empty($email) && !empty($password) && !empty($image) && !empty($bio) && !empty($github) && !empty($linkedin) && !empty($address)){
		echo $per = "<div class='green-per'><i class='fa fa-check-circle'></i> 100%</div>";
	} else {
        $per = $name + $email + $image + $password + $bio + $github + $linkedin + $address;
        $split = explode(".", $per);
