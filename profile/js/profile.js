...
function add_github_account(github_val){
	var github_val = $.trim(github_val);
	var github_url = /^(http|https)\:(\/\/)(www)\.github\.com(\/)[a-zA-Z0-9]+$/;
	if(github_val.length == ""){
		$(".github-error").html('GitHub Field is required');
		$("#add_github").css("border-color","red");
	} else if(github_url.test(github_val)){
		$.ajax({
			type : 'POST',
			url  : 'ajax/profile.php?add_github=true',
			data : {'github_val': github_val},
			dataType : 'JSON',
			success : function(feedback){
				if(feedback['error'] == 'success'){
					location = 'index.php';
		...
		$(".github-error").html('Invalid GitHub URL');
		$("#add_github").css("border-color","red");
	}
}
...
function add_linkedin_url(linkedin_val){
	...
	var linkedin_url = /^(http|https)\:(\/\/)(www)\.linkedin\.com(\/)in(\/)[a-zA-Z0-9]+$/;
