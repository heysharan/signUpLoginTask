...
function add_github_account(){
	GLOBAL $db;
	if(isset($_GET['add_github']) && $_GET['add_github'] == 'true'){
		$github_val = $_POST['github_val'];
		$user_id = $_SESSION['user_id'];
		...
		$r->github;
		if(empty($r->github)){
			$Insert = $db->prepare("UPDATE users SET github = ? WHERE id = ?");
			$Insert->execute(array($github_val, $user_id));
			if($Insert){
				$_SESSION['github_success'] = '<i class="fa fa-check-circle"></i> Your GitHub account is successfully added';
				echo json_encode(array('error' => 'success'));
			} else {
				echo json_encode(array('error' => 'error'));
			}
			...
			$Insert->execute(array($github_val, $user_id));
			if($Insert){
				$_SESSION['github_success'] = '<i class="fa fa-check-circle"></i> Your GitHub account is successfully Updated';
				echo json_encode(array('error' => 'success'));
			} else {
				echo json_encode(array('error' => 'error'));
			}
